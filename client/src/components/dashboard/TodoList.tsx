...
export default function TodoList(props: { className?: string }) {
    ...
    async function removeTask(task: string) {
        if(!(await modal?.CreateModal("Delete this task?", "This action cannot be undone","Delete", "Cancel")))
        {
            return;
        }
        setTaskList((prev) => {
    ...
    async function removeCompletedTasks() {
        if(!(await modal?.CreateModal("Delete all completed tasks?", "This action cannot be undone","Delete", "Cancel")))
        {
            return;
        }
        setTaskList((prev) => {
    ...
    return (
        <div className={`${props.className} flex flex-col justify-between`}>
                        ...
                        return (
                            <div key={index} className="flex items-center mt-4 justify-between">
                                <div className="h-full flex gap-4 items-center">
                                    ...
                                    <label
                                        htmlFor={`${task}`}
                                        className={`${taskList[task] ? "after:origin-left after:scale-100 opacity-50" : "after:origin-right after:scale-0"} after:transition-transform transition-all relative after:bg-text after:absolute after:w-full after:h-[2px] after:left-0 after:top-1/2 after:-translate-y-1/2 after:duration-300`}
                                    >
                                        {task}
                                    </label>
                                </div>
            ...
            <div className="flex gap-1">
                <Input className="w-full" placeHolder="Task" value={newTask} hasError={hasError} onChange={(e) => {
                    setHasError(false);
                ...
                >
                    <svg className="text-text/70 transition-all" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <path fillRule="evenodd" clipRule="evenodd" d="M21.0001 6H3.00006V9C4.10463 9 5.00006 9.89543 5.00006 11V15C5.00006 17.8284 5.00006 19.2426 5.87874 20.1213C6.75742 21 8.17163 21 11.0001 21H13.0001C15.8285 21 17.2427 21 18.1214 20.1213C19.0001 19.2426 19.0001 17.8284 19.0001 15V11C19.0001 9.89543 19.8955 9 21.0001 9V6ZM10.5001 11C10.5001 10.4477 10.0523 10 9.50006 10C8.94778 10 8.50006 10.4477 8.50006 11V16C8.50006 16.5523 8.94778 17 9.50006 17C10.0523 17 10.5001 16.5523 10.5001 16V11ZM15.5001 11C15.5001 10.4477 15.0523 10 14.5001 10C13.9478 10 13.5001 10.4477 13.5001 11V16C13.5001 16.5523 13.9478 17 14.5001 17C15.0523 17 15.5001 16.5523 15.5001 16V11Z" fill="currentColor" />
                        <path d="M10.0681 3.37059C10.1821 3.26427 10.4332 3.17033 10.7825 3.10332C11.1318 3.03632 11.5597 3 12 3C12.4403 3 12.8682 3.03632 13.2175 3.10332C13.5668 3.17033 13.8179 3.26427 13.9319 3.37059" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                    </svg>
                </button>
